...
Let's start by dowloading the monthly OLR data using Python. We'll use the =os= and =urllib.request= modules to do this. Due to its course resolution and lack of a vertical dimension, it isn't a cumbersome dataset (21 MB), but there are still over 40 years of data, spanning from 1979 to the present, 2021.
...
** TODO See if we can find a way to identify the most recent file sitting in the NCEI directory
** TODO Download the daily files
...
** TODO Figure out how to remove white strips from edges of plot.
